/* Persistence layer: all app state lives in localStorage under one key. */
(function (root) {
  var STORAGE_KEY = 'meitalMathQuest_v1';

  var DEFAULT_REWARDS = [
    { id: 'r1', name: 'Extra 15 min of screen time', cost: 300 },
    { id: 'r2', name: 'Pick the family movie night pick', cost: 500 },
  ...
  function createDefaultState() {
    var skills = {};
    root.App.Questions.SKILLS.forEach(function (s) {
      skills[s.id] = { mastery: 30, attempts: 0, correct: 0, lastPracticed: null, masteredAt: null, diagnosticCorrect: null, struggledEarly: false };
    });
    return {
      version: 1,
      studentName: 'Meital',
      ...
      badges: [],
      skills: skills,
      dailyGoal: { sprintsToday: 0, date: todayStr(), goalMet: false, target: 2 },
  ...
  function load() {
    try {
      ...
      Object.keys(def.skills).forEach(function (id) {
        if (!state.skills[id]) state.skills[id] = def.skills[id];
      });
      ...
      if (state.dailyGoal.date !== todayStr()) {
        state.dailyGoal = { sprintsToday: 0, date: todayStr(), goalMet: false, target: state.dailyGoal.target || 2 };
      }
